...
export async function GET(request: Request) {
  ...
  if (!code || !await verifyState(request, state)) return Response.redirect(dashboardUrl(request, "invalid_state"));
  if (!clientId || !clientSecret || !redirectUri || !guildId) return Response.redirect(dashboardUrl(request, "configuration"));
  ...
  try {
    ...
    if (!tokenResponse.ok) return Response.redirect(dashboardUrl(request, "token_exchange"));
    ...
    if (!userResponse.ok || !guildsResponse.ok) return Response.redirect(dashboardUrl(request, "profile"));
    ...
    if (!guild || (permissions & BigInt(8)) === BigInt(0) && (permissions & BigInt(32)) === BigInt(0)) {
      return Response.redirect(dashboardUrl(request, "forbidden"));
    }
    ...
    const response = Response.redirect(dashboardUrl(request));
    response.headers.append("Set-Cookie", clearStateCookie());
    response.headers.append("Set-Cookie", await createSessionCookie(session));
    return response;
  } catch {
    return Response.redirect(dashboardUrl(request, "unexpected"));
  }
}
