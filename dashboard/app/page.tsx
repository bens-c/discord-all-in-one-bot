...
function ControlCenter({ data }: { data: DashboardData }) {
  ...
  return <section id="controls" className="dashboard-card mt-4 scroll-mt-24 overflow-hidden p-5 sm:p-6">
    ...
    <Tabs defaultValue="moderation" className="mt-5"><TabsList className="h-auto w-full flex-wrap justify-start"><TabsTrigger value="moderation"><Gavel />Moderation</TabsTrigger><TabsTrigger value="channels"><Send />Kanäle</TabsTrigger><TabsTrigger value="economy"><Banknote />Economy & XP</TabsTrigger><TabsTrigger value="settings"><Settings2 />Bot-Setup</TabsTrigger></TabsList>
      <TabsContent value="moderation" className="mt-5"><div className="grid gap-4 lg:grid-cols-2"><div className="control-panel"><h4 className="font-medium">Mitglied auswählen</h4><div className="mt-3 space-y-3"><Picker value={memberId} onChange={setMemberId} placeholder="Mitglied wählen" items={data.members} /><Input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Grund für das Audit-Log" maxLength={400} /><div className="flex flex-wrap gap-2"><Button disabled={busy || !memberId} onClick={() => void run("timeout", { minutes: amount })}>Timeout</Button><Button variant="outline" disabled={busy || !memberId} onClick={() => void run("untimeout")}>Timeout entfernen</Button><DangerAction label="Kicken" description="Das Mitglied wird vom Server entfernt." disabled={busy || !memberId} onConfirm={() => void run("kick")} /><DangerAction label="Bannen" description="Das Mitglied wird gebannt und die letzten Nachrichten werden gelöscht." disabled={busy || !memberId} onConfirm={() => void run("ban")} /></div><label className="field-label">Timeout-Minuten<Input type="number" min={1} max={40320} value={amount} onChange={(event) => setAmount(Number(event.target.value))} /></label></div></div><div className="control-panel"><h4 className="font-medium">Rollenverwaltung</h4><div className="mt-3 space-y-3"><Picker value={memberId} onChange={setMemberId} placeholder="Mitglied wählen" items={data.members} /><Picker value={roleId} onChange={setRoleId} placeholder="Rolle wählen" items={selectableRoles} /><div className="flex flex-wrap gap-2"><Button disabled={busy || !memberId || !roleId} onClick={() => void run("role-add")}>Rolle hinzufügen</Button><Button variant="outline" disabled={busy || !memberId || !roleId} onClick={() => void run("role-remove")}>Rolle entfernen</Button></div></div></div></div></TabsContent>
      ...
      <TabsContent value="economy" className="mt-5"><div className="grid gap-4 lg:grid-cols-2"><div className="control-panel"><h4 className="font-medium">Coins und XP anpassen</h4><div className="mt-3 space-y-3"><Picker value={memberId} onChange={setMemberId} placeholder="Mitglied wählen" items={data.members} /><Input type="number" value={amount} onChange={(event) => setAmount(Number(event.target.value))} placeholder="Positiv hinzufügen, negativ abziehen" /><div className="flex gap-2"><Button disabled={busy || !memberId || !data.botState.available} onClick={() => void run("economy-adjust")}>Coins ändern</Button><Button variant="outline" disabled={busy || !memberId || !data.botState.available} onClick={() => void run("xp-adjust")}>XP ändern</Button></div></div></div><div className="control-panel"><h4 className="font-medium">Bot-Daten</h4><div className="mt-3 grid grid-cols-3 gap-3"><Metric label="Warnungen" value={data.botState.warnings || 0} /><Metric label="Giveaways" value={data.botState.activeGiveaways || 0} /><Metric label="Profile" value={(data.botState.levels?.length || 0)} /></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><Ranking title="Top Coins" rows={(data.botState.economy || []).map((item) => ({ id: item.id, value: `${formatNumber(item.total)} 🪙` }))} members={data.members} /><Ranking title="Top XP" rows={(data.botState.levels || []).map((item) => ({ id: item.id, value: `${formatNumber(item.xp)} XP` }))} members={data.members} /></div></div></div></TabsContent>
